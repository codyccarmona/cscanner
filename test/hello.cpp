...
int scan(char *lexeme) {
    ...
    if (skip() == EOF)
        ...
        if(cur == QUOTE || cur == DITTO){
            ...
            /* Check for str token */
                ...
                if(cur == DITTO){            
                    ...
                    //std::fprintf(stderr ,"missing %c for %s on line %i\, DITTO, lexeme, (lexeme[i - 1] == LF ? line_number - 1 : line_number));
        ...
        /* Checks for all tokens that are not char, number, ident, or string */
            ...
            //std::sprintf(error ,"illegal characer: %c on line %i", cur, line_number);
            std::cerr << "illegal characer: " << (char)cur << " on line " << line_number << std::endl;
            lexeme[0] = 0;
            advance();
        }
    }
